...
fb_API_access_token = "CAACEdEose0cBAJy8cyUbgP1eINbui7Urw3N8b2RIkXcac2yWidWfhIROTxHb1noZCcgTzNXBKKMkLeYDiBdlQlUVl1LDzyB834hLTCVOwJLyfA4hQpFMIKBcZCaRFBLNpho6sNVWyCIZCiuYP0bSTTlnlku7pCeTr4YMxClHwxmAsurL2miC2SDvkjeeFZAWZBQiXZApR82bjzL3S9fs9i82taZAOcR04EZD"
...
def count_group_likes():
    ...
    standings_file = open('standings.csv', 'w')
    standings_file.write("Rank,Name,Likes,LikesPerWeek,Comments,CommentsPerWeek,LikesPerComment,Posts,FirstPost\n")
    for i, x in enumerate(most_common_people):
        ...
        rowItem = str(i + 1) + ',' + fullName + "," + str(x[1]) + "," + str(weighted_avg) + "," + str(total_comments_counter[x[0]]) + "," + str(comments_per_week) + "," + str(likesPerComment) + "," + str(total_posts_counter[x[0]]) + "," + join_date.strftime('%Y-%m-%d')
        try:
            print rowItem
            standings_file.write(rowItem.encode('utf8') + "\n")
